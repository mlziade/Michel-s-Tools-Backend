...
export class EasyOcrService {
    ...
    async extractTextFromPdf(file: Express.Multer.File, language: EasyOcrAvailableLanguages): Promise<string> {
        const baseUrl = process.env.OCR_URL;
        if (!baseUrl) {
            throw new Error('OCR_URL is not defined');
        }

        const finalUrl = `${baseUrl}/ocr/pdf`;
        ...
        try {
            ...
            console.log("🚀 ~ EasyOcrService ~ extractTextFromPdf ~ error:", error)
